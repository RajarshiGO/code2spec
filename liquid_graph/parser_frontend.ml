...
type ocaml_type_summary =
  ...
  | OTVar of string
  | OTUser of string
  | OTTyCon of string * ocaml_type_summary list
...
let rec extract_type_summary (ty : Types.type_expr) =
  ...
  | Tconstr (path, args, _) ->
    ...
    let sum_args = List.map extract_type_summary args in
    OTTyCon (name, sum_args)
...
let rec summary_to_ast (s : ocaml_type_summary) : Ast.basetype =
  ...
  | OTTyCon (s, args) -> RTyCon (s, List.map summary_to_ast args)
...
let rec translate_expr (texp : Typedtree.expression) : Ast.expr =
  ...
  | Texp_function (params, body) ->
      ...
      List.iter2 (fun pname pty ->
        Printf.printf "[Parser] Parameter '%s' has type: %s\n" pname (string_of_basetype pty)
...
(* Parse a BSR spec string of the form:
     "forall u v, <body>"   or   "v >= 0"  (simple, current format)
   Returns: either a plain expr (current) or a forall-wrapped one *)
let parse_spec_string (spec_str : string) (param_names : string list) : Ast.expr =
  let spec_str = String.trim spec_str in
  (* Check if it starts with "forall" *)
  if String.length spec_str >= 6 &&
     String.sub spec_str 0 6 = "forall" then begin
    (* Parse: "forall u v, body" *)
    let rest = String.trim (String.sub spec_str 6 (String.length spec_str - 6)) in
    (* Split on comma to get "u v" and body *)
    ...
    (* Build: let u = 0 in let v = 0 in <body> using params *)
    let env_source =
      (List.map (fun p -> Printf.sprintf "let %s = 0 in " p) param_names
       |> String.concat "")
      ^ (List.map (fun q -> Printf.sprintf "let %s = 0 in " q) qvars
         |> String.concat "")
    in
    let full_source = env_source ^ "(" ^ String.trim body_str ^ ")" in
    let body_expr = parse_expr full_source in
    (* Wrap in EForall — we add this to AST *)
